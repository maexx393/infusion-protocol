...
echo -e "${BLUE}🚀 Starting REAL EVM to Algorand Cross-Chain Swap${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""

# Check if we're in the right directory
if [ ! -f "production-evm2algorand-real.ts" ]; then
    echo -e "${RED}❌ Error: production-evm2algorand-real.ts not found${NC}"
    echo "Please run this script from the backend/cross-chain directory"
    exit 1
fi
...
echo -e "${BLUE}🔄 Running REAL EVM to Algorand cross-chain swap...${NC}"
echo ""
...
echo ""
echo -e "${GREEN}✅ REAL EVM to Algorand cross-chain swap completed!${NC}"
echo ""
