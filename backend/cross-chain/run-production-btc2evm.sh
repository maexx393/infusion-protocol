...
# Production BTC to EVM Cross-Chain Swap Script
...
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║                                                               ║"
echo "║  🚀 PRODUCTION CROSS-CHAIN SWAP: BTC ↔ EVM                  ║"
...
echo -e "  ✅ Sufficient POL balance in Carol's wallet (for escrow deposit)"
...
if [ ! -f "production-btc2evm.ts" ]; then
    echo -e "${RED}${CROSS} production-btc2evm.ts not found${NC}"
...
echo -e "\n${BOLD}${ROCKET} Executing Production BTC to EVM Swap...${NC}"
echo -e "${CYAN}Running: ts-node production-btc2evm.ts${NC}"

# Export environment variables
export NODE_TLS_REJECT_UNAUTHORIZED=0

# Run the TypeScript script
ts-node production-btc2evm.ts
