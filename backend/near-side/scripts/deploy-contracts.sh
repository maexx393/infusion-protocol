...
# This script deploys the NEAR contracts for cross-chain swaps
...
build_contracts() {
    ...
    # Navigate to near-contracts directory
    cd ../../near-contracts
...
deploy_escrow() {
    ...
    near deploy "$ESCROW_CONTRACT" ../../near-contracts/fusion-escrow/target/near/fusion_escrow.wasm \
        --initFunction new \
...
deploy_solver() {
    ...
    near deploy "$SOLVER_CONTRACT" ../../near-contracts/fusion-solver/target/near/fusion_solver.wasm \
        --initFunction new \
        --initArgs "{\"owner\": \"$OWNER_ACCOUNT\", \"escrow_contract\": \"$ESCROW_CONTRACT\"}" \
        --accountId "$SOLVER_CONTRACT"
...
deploy_pool() {
    ...
    near deploy "$POOL_CONTRACT" ../../near-contracts/fusion-pool/target/near/fusion_pool.wasm \
        --initFunction new \
        --initArgs "{\"owner\": \"$OWNER_ACCOUNT\", \"solver_contract\": \"$SOLVER_CONTRACT\"}" \
        --accountId "$POOL_CONTRACT"
...
test_contracts() {
    ...
    near call "$ESCROW_CONTRACT" get_statistics --accountId "$OWNER_ACCOUNT" || print_warning "Escrow contract test failed"
    
    # Test solver contract
    print_status "Testing solver contract..."
    near call "$SOLVER_CONTRACT" get_statistics --accountId "$OWNER_ACCOUNT" || print_warning "Solver contract test failed"
    
    # Test pool contract
    print_status "Testing pool contract..."
    near call "$POOL_CONTRACT" get_statistics --accountId "$OWNER_ACCOUNT" || print_warning "Pool contract test failed"
