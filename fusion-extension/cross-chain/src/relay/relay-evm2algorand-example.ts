...
export async function evmToAlgorandExample() {
  const amountAlgo = 0.1;
  const amountEth = 0.01;

  console.log('\n🚀 === EVM to Algorand Order Example ===');
  console.log(`💰 Amount ALGO: ${amountAlgo}`);
  console.log(`💰 Amount ETH: ${amountEth}`);
  
  await pause('Press Enter to start Step 1: Generating Algorand invoice...');
  
  // Step 1: Generate Algorand invoice
  console.log('\n--------------------------------------------');
  console.log('📋 Step 1: Generating Algorand invoice...');
  console.log('--------------------------------------------');
  
  // Generate HTLC secret and hashlock
  const { secret, hashedSecret } = generateAlgorandSecret();
  const algorandInvoice = `algo_invoice_${hashedSecret.substring(0, 16)}`;
  
  console.log(`[USER]⚡ Algorand Invoice: ${algorandInvoice}`);
  ...
  // Step 2: Process EVM to Algorand order through relay
  console.log('\n--------------------------------------------');
  console.log('📋 Step 2: Processing order through relay...');
  console.log('--------------------------------------------');
  const relay = new Relay();
  
  const evmToAlgorandOrder = new OrderEVM2Algorand(
    amountAlgo, // amountAlgo
    algorandInvoice, // algorandInvoice
    amountEth // amountEth
  );
  
  const evmToAlgorandResponse = await relay.processOrderEVM2Algorand(evmToAlgorandOrder);
  console.log('📋 EVM to Algorand Response:', evmToAlgorandResponse);
  ...
  console.log('\n--------------------------------------------');
  console.log('✅ EVM to Algorand example completed!');
  console.log('--------------------------------------------');
}

// Run the example if this file is executed directly
if (require.main === module) {
  console.log('🎯 Cross-Chain Relay Demo - EVM to Algorand Example...\n');
  evmToAlgorandExample()
    .then(() => {
      console.log('\n🎉 EVM to Algorand demo completed successfully!');
      process.exit(0);
    })
    .catch(async (error) => {
