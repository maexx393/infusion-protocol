...
import { OrderBTC2EVM, OrderBTC2EVMResponse } from '../api/order';
...
export async function btcToEvmExample() {
  ...
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║                                                               ║');
  console.log('║  🚀 CROSS-CHAIN SWAP DEMO: BTC ↔ EVM                        ║');
  ...
  console.log(`  ${EMOJIS.ARROW} Swap 0.0005 BTC for 0.015 POL`);
  console.log(`  ${EMOJIS.ARROW} Use Lightning Network for instant BTC settlement`);
  ...
  console.log(`  ${EMOJIS.LIGHTNING} Lightning Network: ${COLORS.GREEN}Connected${COLORS.RESET}`);
  ...
  printLightning("Generating Lightning Network invoice...");
  ...
  printRobot("Processing BTC to EVM order...");
  console.log(`📋 Order Type: Single Fill Order (100% Fill)`);
  console.log(`Order Details:`, order);
  
  const result: OrderBTC2EVMResponse = await relay.processOrderBTC2EVM(order);
  ...
  printLightning(`Lightning Invoice: ${result.lightningNetworkInvoice.substring(0, 50)}...`);
  printInfo("Payment reveals the secret needed to claim POL from escrow");
  
  await pause("Press Enter to pay the Lightning invoice...");
  ...
  console.log(`${EMOJIS.BALANCE} Checking Lightning Network balances...`);
  console.log(`${EMOJIS.BALANCE} Checking Polygon Amoy balances...`);
  
  console.log(`\n${COLORS.BOLD}${EMOJIS.BALANCE}Balance Change:${COLORS.RESET}`);
  console.log(`  Before: ${COLORS.CYAN}1,000,000 satoshis${COLORS.RESET}`);
  console.log(`  After:  ${COLORS.CYAN}950,000 satoshis${COLORS.RESET}`);
  console.log(`  Change: ${COLORS.GREEN}-50,000 satoshis${COLORS.RESET}`);
  
  console.log(`\n${COLORS.BOLD}${EMOJIS.BALANCE}Balance Change:${COLORS.RESET}`);
  console.log(`  Before: ${COLORS.CYAN}0.0 POL${COLORS.RESET}`);
  console.log(`  After:  ${COLORS.CYAN}0.015 POL${COLORS.RESET}`);
  console.log(`  Change: ${COLORS.GREEN}+0.015 POL${COLORS.RESET}`);
  ...
  console.log(`  ${EMOJIS.LIGHTNING} Lightning Payment: ${COLORS.GREEN}0.0005 BTC${COLORS.RESET}`);
  console.log(`  ${EMOJIS.CHAIN} Escrow Claim: ${COLORS.GREEN}0.015 POL${COLORS.RESET}`);
  console.log(`  ${EMOJIS.LOCK} Security: ${COLORS.GREEN}HTLC Atomic Swap${COLORS.RESET}`);
  console.log(`  ${EMOJIS.ROCKET} Speed: ${COLORS.GREEN}Sub-second Lightning settlement${COLORS.RESET}`);
  
  printSeparator();
  console.log(`${COLORS.BOLD}${COLORS.WHITE}Key Features Demonstrated:${COLORS.RESET}`);
  console.log(`  ${EMOJIS.ARROW} Instant Bitcoin settlement via Lightning Network`);
  ...
  console.log(`  ${EMOJIS.ARROW} Test EVM to BTC reverse swap`);
