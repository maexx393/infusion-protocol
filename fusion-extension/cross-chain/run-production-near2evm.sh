#!/bin/bash
# Production NEAR to EVM Cross-Chain Swap Script
set -e
...
display_header() {
    ...
    echo "╔═══════════════════════════════════════════════════════════════╗"
    echo "║                                                               ║"
    echo "║  🚀 PRODUCTION CROSS-CHAIN SWAP: NEAR ↔ EVM                 ║"
    echo "║  ⚡ NEAR Protocol ↔ Polygon Amoy                            ║"
    ...
    echo -e "  ${ARROW}  Real blockchain transactions on NEAR Protocol"
    ...
    echo -e "  ${CHECKMARK} Sufficient NEAR balance in Alice's wallet"
    echo -e "  ${CHECKMARK} Network connectivity to NEAR Protocol"
}
...
check_environment() {
    ...
    if [ ! -f "production-near2evm.ts" ]; then
        print_error "production-near2evm.ts not found"
...
run_production_script() {
    print_status "Starting NEAR to EVM cross-chain swap..."
    echo ""
    
    # Run the TypeScript script
    ts-node production-near2evm.ts
}

# Function to display completion message
display_completion() {
    echo ""
    print_success "NEAR to EVM cross-chain swap completed!"
    echo ""
    ...
    echo -e "  ${ARROW} Test EVM to NEAR reverse swap"
...
main() {
    ...
    echo ""
    echo -e "${BOLD}${YELLOW}Press Enter to start the NEAR to EVM cross-chain swap...${NC}"
    read -r
