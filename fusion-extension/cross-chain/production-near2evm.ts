...
async function runProductionDemo() {
  try {
    ...
    printUser('Creating swap order: 0.1 NEAR → 0.0005 POL');
    ...
    const depositParams = {
      amountEth: 0.0005, // Reduced to work with current balances
      hashedSecret,
    ...
    printTransactionDetails(depositResult.txHash, '0.0005 POL', 'Polygon Amoy');
    ...
    printTransactionDetails(claimResult.txHash, '0.0005 POL', 'Polygon Amoy');
    ...
    printBalanceChanges('0.0', '0.0005', '+0.0005', 'POL (Alice)');
    printBalanceChanges('0.0005', '0.0', '-0.0005', 'POL (Carol)');
    ...
    console.log(`  ${CHAIN} EVM Escrow Claim: ${GREEN}0.0005 POL${NC}`);
